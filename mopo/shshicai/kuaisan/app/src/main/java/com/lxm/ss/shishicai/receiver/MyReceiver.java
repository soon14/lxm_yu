package com.lxm.ss.shishicai.receiver;
...
import com.lxm.ss.shishicai.ui.main.DetailContentActivity;
...
public class MyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		try {
			...
			Zlog.ii(TAG+"[MyReceiver] onReceive - " + intent.getAction() + ", extras: " + printBundle(bundle));

			if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
				String regId = bundle.getString(JPushInterface.EXTRA_REGISTRATION_ID);
				Zlog.ii(TAG+ "[MyReceiver] 接收Registration Id : " + regId);
				//send the Registration Id to your server...

			} else if (JPushInterface.ACTION_MESSAGE_RECEIVED.equals(intent.getAction())) {
				Zlog.ii(TAG+ "[MyReceiver] 接收到推送下来的自定义消息: " + bundle.getString(JPushInterface.EXTRA_MESSAGE));
				processCustomMessage(context, bundle);

			} else if (JPushInterface.ACTION_NOTIFICATION_RECEIVED.equals(intent.getAction())) {
				Zlog.ii(TAG+ "[MyReceiver] 接收到推送下来的通知");
				int notifactionId = bundle.getInt(JPushInterface.EXTRA_NOTIFICATION_ID);
				Zlog.ii(TAG+ "[MyReceiver] 接收到推送下来的通知的ID: " + notifactionId);
...
//				context.startActivity(i);
				...
				Zlog.ii(TAG+ "[MyReceiver] 用户收到到RICH PUSH CALLBACK: " + bundle.getString(JPushInterface.EXTRA_EXTRA));
				//在这里根据 JPushInterface.EXTRA_EXTRA 的内容处理代码，比如打开新的Activity， 打开一个网页等..

			} else if(JPushInterface.ACTION_CONNECTION_CHANGE.equals(intent.getAction())) {
				boolean connected = intent.getBooleanExtra(JPushInterface.EXTRA_CONNECTION_CHANGE, false);
				Zlog.ii(TAG+ "[MyReceiver]" + intent.getAction() +" connected state change to "+connected);
			} else {
				Zlog.ii(TAG+ "[MyReceiver] Unhandled intent - " + intent.getAction());
			}
		} catch (Exception e){
		...
		for (String key : bundle.keySet()) {
			if (key.equals(JPushInterface.EXTRA_NOTIFICATION_ID)) {
				...
				if (TextUtils.isEmpty(bundle.getString(JPushInterface.EXTRA_EXTRA))) {
					Zlog.ii(TAG+"This message has no Extra data");
					continue;
				}
				...
				try {
					...
					Zlog.ii(TAG+ "Get message extra JSON error!");
				}
	...
	private void processCustomMessage(Context context, Bundle bundle) {
		...
		Zlog.ii("lxm processCustomMessage:" +message +"  " + extras );
